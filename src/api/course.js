...
export async function getAllCourseNew(pageNumber, pageSize) {
    return handleApiRequest(async () => {
        return await instance.get(`${URL_COURSE}/asc/dateUpload/page/${pageNumber}/${pageSize}`);
    });
}

export async function findAllCourseByName(courseName, pageNumber, pageSize) {
    return handleApiRequest(async () => {
        return await instance.get(`${URL_COURSE}/search/name/${courseName}/page/${pageNumber}/${pageSize}`);
    });
}
...
export async function isBuyCourse(courseId, userId) {
    return handleApiRequest(async () => {
        return await instance.get(`${URL_COURSE}/buying/course/${courseId}/user/${userId}`);
    })
}
